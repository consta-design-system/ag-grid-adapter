...
export const HeaderCell = (props: Props) => {
  ...
  const syncExpandButtons = () => {
    setIsExpanded[columnGroup?.getProvidedColumnGroup().isExpanded() ? 'on' : 'off']()
  }
  ...
  useEffect(() => {
    ...
    onSortChanged()
    columnGroup?.getProvidedColumnGroup().addEventListener('expandedChanged', syncExpandButtons)
    syncExpandButtons()
    return () => {
  ...
  return (
    <div className={cnHeaderCell({ size, align })}>
      <div className={cnHeaderCell('Container')}>
        ...
        {isExpandable && (
          <button type="button" onClick={changeExpand} className={cnHeaderCell('GroupButton')}>
            {isExpanded ? (
      ...
      {enableMenu && (
        <button className={cnHeaderCell('Menu')} ref={menuRef} onClick={onMenuClick} type="button">
