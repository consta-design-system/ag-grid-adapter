...
Компонент наложеия, который может быть как и лоудером, так и содержать сообщение
...
| ---------- | ---------------------- | ----------------------- | ------------------- |
| `size?`    | `'s', 'm'`             | `m`                     | Размер              |
| `type?`    | `'loading', 'message'` | `loading`               | Тип контента        |
| `message?` | `string`               | `Результаты не найдены` | Текст сообщения     |
| `icon?`    | `React.FC<IconProps>`  | -                       | Отображаемая иконка |
